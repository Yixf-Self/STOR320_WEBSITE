---
title: "Analysis 2 Solutions: Joining Cholesterol with Crimes and Web Scraping Wikipedia"
...
**Due:** Friday, October 12, 2018 by 5:00 PM
...
```{r}
CHOL1a = CHOL1 %>% rename(`4`=After4weeks,`8`=After8weeks)
names(CHOL1a)
```
...
```{r}
CHOL1b = CHOL1a %>% gather(`4`:`8`,key=Week,value=Response,convert=T)
str(CHOL1b)
```
...
Join the nutritional facts of the different margarine brands in `CHOL2a` to the experimental results in `CHOL1b`. Create a new data frame named `CHOL.COMBINED` and display the table using `head(CHOL.COMBINED)`. This final data frame should contain *36* observations and *10* variables.
...
Create a new data frame named `VIOLENT3` that connects the population change and density measures from 2018 contained in `CENSUS` to the cities and states in `VIOLENT2`. Use `head(VIOLENT3)` to give a preview of the new merged dataset. Examine the resulting data frame `VIOLENT3` for cities and states missing information from `CENSUS`. In a complete sentence, identify any locations missing census information.
...
Answer *(2 Points)*: Mobile, Alabama is the only city that is missing census information. 


###Q3 *(4 Points)*

Either ambitiously use one step or less-ambitiously use multiple steps to add the longitude and latitude information provided in `ZIP` to the cities and states in `VIOLENT3`. You will need to use `STATE_ABBREV` data to link these two data frames. Your final data frame named `FINAL.VIOLENT` should contain all of the information in `VIOLENT3` along with the variables `lat` and `lon` from `ZIP`. There should be **no** state abbreviations in `FINAL.VIOLENT` since this information is redundant. Use `str(FINAL.VIOLENT)` to demonstrate that everything worked as planned out by your imperial commander Dr. Mario. In `FINAL.VIOLENT` identify what cities are missing latitude and longitude. Closely, inspect both the `ZIP` and `VIOLENT3` data frames. Report the cities missing geographical locations and explain in complete sentences why this happened. Then, challenge yourself and attempt to fix this in a new data frame called `FINAL.VIOLENT.FIX`. Using a combination of `str()` and `filter()`, only display the rows in `FINAL.VIOLENT.FIX` for the cities that `FINAL.VIOLENT` was missing latitude and longitude.
...
Answer *(1 Points)*: St. Louis, Missouri, St. Paul, Minnesota, and St. Petersburg, Florida are all missing latitude and longitude. This is because these cities have the abbreviation "St." spelled out in ZIP

Code and Output *(1 Point)*:
```{r}
ZIP.FIX = ZIP %>%
            mutate(city = ifelse(city=="Saint Louis" & state=="MO","St. Louis", city)) %>%
            mutate(city = ifelse(city=="Saint Paul" & state=="MN","St. Paul", city)) %>%
            mutate(city = ifelse(city=="Saint Petersburg" & state=="FL","St. Petersburg", city))
FINAL.VIOLENT.FIX = left_join(left_join(VIOLENT3,STATE_ABBREV),ZIP.FIX,by=c("City"="city","state"="state")) %>%
                    select(-state)
str(filter(FINAL.VIOLENT.FIX,City %in% c("St. Louis","St. Paul", "St. Petersburg")))
```
...
Answer *(2 Points)*: https://en.wikipedia.org/wiki/Salary_cap#In_Australia_and_New_Zealand
...
Answer *(4 Points)*: I am web scraping the table that provides the MLB teams since 2003 that have been subject to salary tax. The variables included are team name, years liable, and total tax paid.
...
```{r}
URL = "https://en.wikipedia.org/wiki/Salary_cap#In_Australia_and_New_Zealand"
DATA = URL %>%
        read_html() %>%
