...
Answer *(2 Points)*: Lexington, KY, is missing census information.
...
Finally, challenge yourself and attempt to fix this problem in a new data frame called `FINAL.VIOLENT.FIX`. Use a combination of `str()` and `filter()` to only display the data in `FINAL.VIOLENT.FIX` for the location(s) that `FINAL.VIOLENT` was missing latitude and longitude. Do this in the second code chunk below. Do this in the second code chunk below.
...
Answer *(1 Points)*: Washington, DC is missing latitude and longitude because there is no District of Columbia abbreviation in `STATE_ABBREV`. Also, city name for Washington DC is named "Washington" in `ZIP`.

Code and Output *(1 Point)*:
```{r}
STATE_ABBREV.FIX = rbind(STATE_ABBREV,c("District Of Columbia","DC"))
ZIP.FIX = mutate(ZIP,city=ifelse(city=="Washington" & state=="DC","Washington Dc",city))
FINAL.VIOLENT.FIX = left_join(left_join(VIOLENT4,STATE_ABBREV.FIX,by="State"),ZIP.FIX,by=c("City"="city","state"="state")) %>%
                    select(-state)
str(filter(FINAL.VIOLENT.FIX,City %in% c("Washington Dc")))
```
...
Answer *(2 Points)*: I am web scraping the table that provides the MLB teams since 2003 that have been subject to salary tax. The variables included are team name, years liable, and total tax paid. This is the third table on the webpage.
