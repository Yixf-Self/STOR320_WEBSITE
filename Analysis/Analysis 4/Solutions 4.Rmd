---
title: "Analysis 4 Solutions: Model Selection Via K-Fold CV"
...
**Due:** Friday, November 23, 2018 by 5:00 PM
...
What is the point of stealing your tuition, if I cannot grab some fish? The model below is an expression of a family of polynomial regression models that use `A` and `D` to explain the variation in `W`. Every choice of $I$ and $J$ leads to a different model. For our purpose, we would like to consider all possible subset models where $I\leq 10$ and $J\leq 10$. To choose the best model, we rely on cross-validation (CV) to estimate out-of-sample root mean squared error (RMSE). The best choice of $I$ and $J$ minimize prediction error. I highly recommend seeing [Tutorial 13](https://github.com/SuperMarioGiacomazzo/STOR320_WEBSITE/tree/master/Tutorial/Tutorial%2013) which provides necessary code to complete this assignment. If you don't look at this tutorial, you will be miserable like these fish.
...
In [Tutorial 13](https://github.com/SuperMarioGiacomazzo/STOR320_WEBSITE/tree/master/Tutorial/Tutorial%2013), I demonstrated how to use 10-Fold CV to obtain an out-of-sample RMSE when $I=4$ and $J=3$. In this assignment, we will also use 10 folds. Using `crossv_kfold()`, create a new dataframe called `DATA2` that contains two new variables `train` and `test` that are list-columns. 
...
In the code chunk below, we begin by initiating an empty $10 \times 10$ matrix of missing values called `OUT.RMSE`. Each row corresponds to a different choice of $I$ and each column corresponds to a different choice of $J$. Apply the code in [Tutorial 13](https://github.com/SuperMarioGiacomazzo/STOR320_WEBSITE/tree/master/Tutorial/Tutorial%2013) in a double loop that performs 10-Fold CV to estimate out-of-sample RMSE under each polynomial model where $I \in \{1,2,3, \cdots,10\}$ and $J \in \{1,2,3, \cdots,10\}$ and then saves the RMSE in the $(I,J)$-cell of the matrix `OUT.RMSE`.
...
In the code chunk below, we start by making the information found the summarized information in `OUT.RMSE` tidy. There are three columns in `OUT.RMSE2` that links the cross-validated RMSE to all considered combinations of $I$ and $J$.
...
Now, observe the figure below that shows the change in maximum water temperature ($W$) across Julian days ($D$) .
...
Using `mutate()`, we create a tibble `BEST5.DATA` based off `DATA`. The new object `BEST5.DATA` contains 5 columns of predictions under the top 5 models based on the values of `I` and `J` in `BEST5.RMSE`. The five columns of predictions should be given names `First`, `Second`, `Third`, `Fourth`,`Fifth`, in order from best to worst. 
...
Create a figure that overlays the predictions for the top 5 models over the raw data. The raw data is shown using `geom_point()`. The predictions should be created using `geom_line()` with different colors for each of the Models. In a complete sentence, explain how the predicted maximum water temperatures differ for the top five models.
...
Answer *(2 Points)*: The predictions all overlap, and there is no visual difference in the predictions across the models.


###Q4 *(3 Points)*

The following two figures show the marginal change in the average out-of-sample RMSE as $I$ and $J$ increase. Based on these figures, what $I$ and $J$ would you recommend going forward? Critically, think about what these figures are telling us. Give a reason for your answer based on these graphics. Answer the question below the two figures in complete sentences.

```{r,fig.width=8,fig.height=4,fig.show='hold',fig.align='center',echo=F}
OUT.RMSE2 %>% 
  group_by(I) %>% 
...
Answer: I would suggest $I$ to be 2 and $J$ to be either 3 or 4. After these choices of $I$ and $J$, the average out-of-sample RMSE seems to become stable and not change much. Even if we obtain better predictions for larger choices of $I$ and $J$, it makes sense to go with the most simple model since it is competively equivalent.

##Part 3: Plots of Best Model

###Q1 *(4 Points)*

I want you to create a simple function called `BEST.func()` that outputs a 2-dimensional vector where the first element is the $I$ and $J$ that corresponds to the lowest RMSE. The output vector should be a numeric vector and not a tibble with factor variables. The only argument called "data" should be a data frame that is identical in format to `OUT.RMSE2`. The last line will process the function when `data=OUT.RMSE2` and save the best choices of `I` and `J` to an object called `BEST.CHOICE` and then print out the 2-dimensional vector. I advise using the function `which.min()`.
