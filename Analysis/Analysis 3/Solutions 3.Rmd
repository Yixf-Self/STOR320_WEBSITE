---
title: "Analysis 3 Solutions: Statistical Programming with Traffic Data"
...
**Assigned:** Friday, October 19, 2018

**Due:** Friday, October 26, 2018 by 5:00 PM
...
Assuming you followed all directions, the code below displays a plot that shows the change in traffic occupancy for all four Mondays at the location `L101`. Different colors are used to identify the different Mondays in the month of April.
```{r,warning=F,eval=T}
...
```{r,eval=T}
DAYPLOT.func(data=DATA2,sensor="L106",dayabbrev="Th")
```
...
for(k in unique(DATA2$DAY)){
  OUTPUT.A=rbind(OUTPUT.A,TRAFFICSUM.func(aprilday=k))
}
...
for(k in unique(DATA2$DAY)){
  j=which(unique(DATA2$DAY)==k)
  OUTPUT.B[j,]=TRAFFICSUM.func(aprilday=k)
}
